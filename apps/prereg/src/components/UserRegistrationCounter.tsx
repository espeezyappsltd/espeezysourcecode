...
export default function UserRegistrationCounter({ registeredCount, goal, authUserCount = 0 }: { registeredCount: number; goal: number; authUserCount?: number }) {
  ...
  const remainingCount = Math.max(goal - registeredCount, 0)
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
        ...
        <span style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}>
          {remainingCount.toLocaleString()} remaining / {goal.toLocaleString()}
        </span>
      </div>
      <div
        role="progressbar"
        ...
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`Registration progress: ${registeredCount.toLocaleString()} registered, ${remainingCount.toLocaleString()} remaining out of ${goal.toLocaleString()}`}
