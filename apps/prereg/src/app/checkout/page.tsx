...
function CheckoutFlow() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #06080f 0%, #0a0a0a 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <AnimatePresence mode="wait">
        {step === 'review' ? (
          <motion.div key="review" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12, scale: 0.97 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} style={{ width: '100%', maxWidth: '480px', position: 'relative', zIndex: 10 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '32px', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
              <div style={{ padding: '2rem 2rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                  <div>
                    <div style={{ fontSize: '0.7rem', fontWeight: 800, color: plan.color, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.25rem' }}>{plan.badge}</div>
              ...
              <div style={{ padding: '0 2rem 2rem' }}>
                <motion.button onClick={handlePay} onHoverStart={() => setHovered(true)} onHoverEnd={() => setHovered(false)} whileTap={{ scale: 0.97 }}
                  style={{ width: '100%', padding: '1.1rem', background: `linear-gradient(135deg, ${plan.color} 0%, ${plan.color}cc 100%)`, border: 'none', borderRadius: '16px', color: '#0a0a0a', fontSize: '0.95rem', fontWeight: 950, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', boxShadow: `0 12px 32px ${plan.color}40`, letterSpacing: '-0.01em' }}>
                  <CreditCard size={18} />
