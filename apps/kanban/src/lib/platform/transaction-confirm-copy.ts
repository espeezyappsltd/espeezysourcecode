...
const SUBSCRIPTION_PLAN_META: Record<string, { label: string; price: string }> = {
  pro: { label: 'Pro Monthly', price: '£2.99/month' },
  premium: { label: 'Premium Partner', price: 'lifetime access' },
  lifetime: { label: 'Lifetime seat', price: 'one-time payment' },
}
...
export function subscriptionCheckoutCopy(plan: string): TransactionConfirmCopy {
  ...
  return {
    ...
    message: `You will be redirected to Stripe for ${meta.label} (${meta.price}).`,
