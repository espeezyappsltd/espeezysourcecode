...
export function listingTypeOf(listing: Pick<Listing, 'listing_type'>): ListingType {
  return listing.listing_type === 'digital' ? 'digital' : 'physical'
}

export function deliveryKindOf(listing: Pick<Listing, 'delivery_kind'>): DeliveryKind {
  const k = listing.delivery_kind
  if (k === 'file' || k === 'link') return k
  return 'meetup'
}

export function quantityRemaining(listing: Pick<Listing, 'quantity_available' | 'quantity' | 'listing_type' | 'status'>): number {
...
export function isDigitalDownloadable(listing: Pick<Listing, 'listing_type' | 'delivery_kind' | 'digital_url' | 'digital_content'>): boolean {
...
export function saleRuleLabel(listing: Pick<Listing, 'listing_type' | 'delivery_kind' | 'quantity' | 'quantity_available'>): string {
