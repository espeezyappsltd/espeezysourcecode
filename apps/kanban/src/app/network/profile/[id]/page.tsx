...
import { isTimestampWithSeconds, type DateInput } from '@/types/common'
...
export default function StudentProfilePage() {
  ...
  const [member, setMember] = useState<(Profile & { groups?: Group | Group[] | null }) | null>(null)
  ...
  const formatRelativeTime = (date: DateInput) => {
    if (!date) return 'Unknown'
    const now = new Date()
    let targetDate: Date
    if (isTimestampWithSeconds(date)) {
      targetDate = new Date(date.seconds * 1000)
    } else {
      targetDate = new Date(date as string | number | Date)
    }
  ...
  if (!member) {
    return (
      <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
        ...
        <Link href="/network" className="btn btn-secondary btn-inline" style={{ marginTop: '1rem' }}>Back to Network</Link>
      </div>
    )
  }
              ...
              <div className="page-fade">
                 ...
                 <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    <p style={{ fontSize: '0.9rem', lineHeight: 1.6, color: 'var(--text-main)', opacity: 0.85 }}>
                      {member.biography || `An active scholar in the Espeezy network, specializing in ${member.course_name || 'their academic field'}.`}
                    </p>
                 </div>
              </div>
           )}
