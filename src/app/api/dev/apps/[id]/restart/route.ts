...
import { restartApp, getAppRuntime } from '@/lib/dev-hub/process-manager'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  restartApp(id, Number.isFinite(port) ? port : undefined)
  return NextResponse.json({ runtime: getAppRuntime(id) })
}
